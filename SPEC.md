...
- 原型提供独立全屏的基础与提前响铃离线演示：基础 fixture 固定为 `2026-09-02 07:30`，提前 fixture 固定为 `07:18`（提前 12 分钟）；两者均以 10 分钟贪睡开始，贪睡时长仅接受 1–30 分钟整数。演示的停止、贪睡、再次响铃与重复操作按演示实例 ID 幂等，但不写入计划、记录或日期覆盖，也不调用 Android 系统能力。
- Android 基础响铃页复用原型的壁纸、字体、时间、原因卡和双按钮布局，展示实例的真实时间、日期与计划名称；系统状态栏和导航区使用平台提供的安全区域，不绘制演示状态栏。
- 原生停止／贪睡点击后进入处理中状态；根据设备保护快照的动作回执与响铃服务的实例集合确认结果。贪睡失败保留父实例响铃并显示重试提示；成功后展示实际子实例时间，子实例到点进入“贪睡后再次响铃”。
...
- 原型的 `ringing-basic` 与 `ringing` 仅用于离线交互验收，分别表示基础与提前 fixture；它们不是 Android 响铃服务、不会播放音频或振动、不会注册系统闹钟，也不会修改 `alarmPlans`、`alarmEvents` 或 `dateOverrides`。提前 fixture 用于离线演示；自动评估验收使用 Android 的真实决策和实例。
...
- `ringing-basic` 展示本 App 基础闹钟 fixture；`ringing` 展示提前 fixture，并显示固定提前量和理由。两者使用独立全屏，不叠加原型外层状态栏。
- 停止与贪睡在原路由展示结果。停止态仅提供“返回闹钟／重新演示”；贪睡态仅提供“返回闹钟／模拟再次响铃”。贪睡产生演示子实例：基础再次响铃为 `07:40`，提前再次响铃为 `07:28`；二次贪睡按所选时长继续递增。
- 子实例显示“贪睡后再次响铃”和“贪睡提醒”，不沿用初始基础／提前量提示；跨日时同步更新日期、星期和时间。
- 离开并重新进入、重置或刷新会重建演示会话。提前 fixture 不展示真实路线、天气或 Provider 评估结果。

### 8.7B Android 真实响铃

- `AlarmRingingActivity` 从设备保护存储读取当前 occurrence；只有 `FIRING` 且该实例存在于响铃服务的活动集合时允许停止／贪睡操作。回执已确认但服务尚未结束时按钮保持禁用；待响、缺失、失败、已错过及已取消实例不显示可操作的响铃状态。
