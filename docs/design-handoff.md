...
> 2026-09-01 高德已接入：Android 已实现授权、加密运行时 Key、地图、单次定位、POI／输入提示、五种路线、最多三条备选、路况和计划覆盖；待用户提供 Web Service Key 与 Android SDK Key 后完成设备实网验收。`prototype/` 使用确定性离线 fixture 验收页面状态。天气和提前计算仍保持留白。
...
|---|---|---|---|
| 首页空态 | `57:480` | `home` | 天气显示“暂未接入”；路线区域按高德 fixture 显示授权／Key／加载／结果状态。 |
...
|---:|---|---|---|---|---|
| 01 | 今日·出行总览 `3:10` | `home` | 8.2 | 首次引导完成或底部“今日”进入；本地闹钟空态进入添加；底部导航不返回上一页。 | 下一次有效本地闹钟、真实注册状态、天气未接入与高德 fixture 状态。 |
...
| 21 | 可靠性诊断 `37:297` | `diagnostics` | 8.8、FR-009、FR-010 | 从设置或异常页进入；重新检查后留在本页；返回设置。 | 通知、精确闹钟、全屏提醒、音量、铃声、天气未接入与高德配置状态和最近本机事件；不显示密钥、完整地址或经纬度。 |
