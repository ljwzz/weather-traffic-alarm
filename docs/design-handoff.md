...
- 原型路由以 `prototype/app.js` 的 `ROUTES` 为准：主页是 `home`，地点选择是 `place-search`。本轮最终 Web 原型核对和 Android／真实服务验收均待根任务汇总；[`prototype/qa/README.md`](../prototype/qa/README.md)只记录已执行的检查，不替代最终验收。
...
| 12 | 响铃·本地闹钟 `3:73` | `ringing` | FR-008 | 原型以场景页模拟：停止回到计划，贪睡后进入锁屏模拟。产品由有效本地实例触发，并按 FR-008 处理停止／贪睡。 | 停止／贪睡按实例 ID 幂等。 |
