...
|---|---|---|---|
| 首页空态 | `57:480` | `home` | 天气与路线仅显示“暂未接入”；本地闹钟空态进入添加。 |
| 天气留白 | `57:635` | `weather` | 不展示模拟预报、温度、天气图或数据来源。 |
| 路线留白 | `57:833` | `route` | 可保存地点文字与出行方式；不展示模拟地图、距离、耗时或路况。 |
...
| 凭据未接入 | `57:1187` | `credentials` | Android Keystore 本地保存／清空；测试显示未接入且不发送请求。 |
...
|---:|---|---|---|---|---|
| 01 | 今日·出行总览 `3:10` | `home` | 8.2 | 首次引导完成或底部“今日”进入；本地闹钟空态进入添加；底部导航不返回上一页。 | 下一次有效本地闹钟、真实注册状态、Provider 未接入说明。 |
| 02 | 天气·预报地图 `3:15` | `weather` | 8.2、8.4 | 从今日天气进入；返回今日。 | 当前仅留白和“暂未接入”，无预报、数据来源或数据时间。 |
| 03 | 路线·上班通勤维护 `3:20` | `route` | 8.4 | 从今日或底部“路线”进入；管理地点文字和出行方式；底部导航离开。 | 无模拟地图、距离、耗时或路线摘要。 |
| 04 | 路线·编辑与选点 `3:25` | `route-edit` | 8.4、FR-011 | 从路线页进入；起点/终点进入 `place-search`；保存返回路线页，返回/取消保留已保存路线。 | 驾车、公交、步行、骑行、电动车五选一；未配置 SDK Key 时隐藏地图与当前位置。 |
...
| 14 | 异常·评估未完成 `14:157` | `failure` | 后续能力 | Provider 评估未接入；不作为当前 Android 页面完成条件。 |
...
| 19 | 数据与凭证 `37:249` | `credentials` | 5.6、8.6、FR-012、FR-013 | 从首次引导或设置进入；保存留在当前页；清空走确认覆盖层；仅页头返回设置。 | 字段掩码；当前测试显示未接入且不发请求；不显示密钥。 |
...
| 21 | 可靠性诊断 `37:297` | `diagnostics` | 8.8、FR-009、FR-010 | 从设置或异常页进入；重新检查后留在本页；返回设置。 | 通知、精确闹钟、全屏提醒、音量、铃声、Provider 未接入状态和最近本机事件；不显示密钥、完整地址或经纬度。 |

## 地点选择子状态

| 状态 / Figma 节点 | 原型 route | 进入、返回与主操作 | 状态与校验 |
|---|---|---|---|
| 地点·搜索与管理 `42:281` | `place-search` | 从路线编辑的起点或终点进入；搜索、选结果/常用地点/当前位置后点击“使用这个地点”回到 `route-edit`；取消返回 `route-edit`。 | 搜索结果、无结果、地图选点、定位不可用均为可切换状态；正式应用仅在用户点按当前位置时请求前台定位，未配置 SDK Key 时隐藏地图和当前位置。 |
