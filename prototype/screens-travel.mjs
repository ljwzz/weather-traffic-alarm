...
import { AMAP_FIXTURE_STATES, CAIYUN_FIXTURE_STATES, amapFixtureState, caiyunFixtureState, resolveCommute } from './state.mjs';
...
export function createTravelScreens({ action, overlayAction, asset, state }) {
  ...
  return {
    home() { const c = read().config || {}; const plans = c.alarmPlans || []; const first = plans.find(plan => plan.enabled); const status = weatherFixture(); const label = ({ loading:'加载中', success:'模拟成功', cached:'模拟缓存', error:'模拟错误' })[status]; return `<div class="travel-home"><button type="button" class="travel-home-weather provider-placeholder provider-home" data-route="weather"><span>⌁</span><strong>彩云天气 · ${label}</strong><p>本地 fixture；不请求真实 API。</p></button><button type="button" class="travel-alarm" data-route="plans"><div class="travel-alarm-top">本地闹钟 <em>${first ? '已创建' : '空列表'}</em></div><div class="travel-alarm-time">${first ? esc(first.time) : '—'} <span>${first ? esc(first.name) : '添加第一个闹钟'}<small>${first ? '由 Android 注册与响铃' : '支持单次、每周和工作日'}</small></span></div><div class="travel-alarm-result"><div>已启用<b>${plans.filter(plan => plan.enabled).length} 个</b></div><div>下一步<b>${first ? '查看闹钟' : '立即添加'}</b></div></button><section class="travel-title"><h2>今天的通勤</h2>${link('查看路线', 'route')}</section>${map('travel-commute')}<p class="travel-assurance">高德地图仅在用户同意后初始化。</p></div>`; },
    weather() { const status = weatherFixture(); if (status === CAIYUN_FIXTURE_STATES.LOADING) return `<div class="travel-weather">${blank('彩云天气加载中', '本地 fixture 正在模拟加载；未发送网络请求。', 'travel-weather-map')}<section class="travel-forecast"><header><h2>天气预报</h2><span>彩云天气 · 模拟</span></header><p>正在等待模拟结果。</p><footer>自动提前评估仍未启用。</footer></section></div>`; if (status === CAIYUN_FIXTURE_STATES.ERROR) return `<div class="travel-weather">${blank('彩云天气暂不可用', '本地 fixture 模拟服务错误；未发送网络请求。', 'travel-weather-map')}<section class="travel-forecast"><header><h2>天气预报</h2><span>彩云天气 · 模拟错误</span></header><p>错误状态不会生成天气缓冲或提前闹钟结果。</p><footer>自动提前评估仍未启用。</footer></section></div>`; const cached = status === CAIYUN_FIXTURE_STATES.CACHED; return `<div class="travel-weather"><section class="caiyun-weather-fixture travel-weather-map" aria-label="彩云天气本地 fixture"><div><span>${cached ? '缓存数据' : '模拟成功'}</span><strong>22°</strong><b>小雨</b><p>降水概率 60% · 体感 21°</p></div><small>彩云天气 · ${cached ? '本地缓存 fixture' : '本地 fixture'}</small></section><section class="travel-forecast"><header><h2>天气预报</h2><span>彩云天气</span></header><p>${cached ? '正在展示缓存的模拟天气数据。' : '正在展示确定性的模拟天气数据。'}</p><div class="travel-hours"><button class="is-selected"><small>现在</small><b>22°</b><em>小雨</em></button><button><small>08:00</small><b>23°</b><em>小雨</em></button><button><small>09:00</small><b>24°</b><em>阴</em></button><button><small>10:00</small><b>25°</b><em>阴</em></button><button><small>11:00</small><b>26°</b><em>多云</em></button></div><footer>自动提前评估仍未启用；基础闹钟不使用天气数据。</footer></section></div>`; },
